...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS){
        	...
        	String t1Name = cat.getTableName(t1Id);
        	String t2Name = cat.getTableName(t2Id);
        	TableStats t1Stats = stats.get(t1Name);
        	TableStats t2Stats = stats.get(t2Name);
        	int field1No = cat.getTupleDesc(t1Id).fieldNameToIndex(field1PureName);
        	int field2No = cat.getTupleDesc(t2Id).fieldNameToIndex(field2PureName);
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size(); i++){
        	for (Set<LogicalJoinNode> subset1: enumerateSubsets(joins, i)){
        		Iterator<LogicalJoinNode> subsetIter = subset1.iterator();
        		if (i == 1){
        			LogicalJoinNode joinToRemove = subsetIter.next();
        			CostCard plan = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, subset1, Double.MAX_VALUE, pc);
        			pc.addPlan(subset1, plan.cost, plan.card, plan.plan);
        		}else{
        			...
        			while (subsetIter.hasNext()){
        				...
        				CostCard plan;
        				if (bestPlan == null){
        					plan = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, subset1, Double.MAX_VALUE, pc);
        				}else{
        					plan = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, subset1, bestPlan.cost, pc);
        				}
        				if (plan != null){
        					bestPlan = plan;
        				}
        			...
        			if (bestPlan != null){
        				pc.addPlan(subset1, bestPlan.cost, bestPlan.card, bestPlan.plan);
        			}else{
        				pc.addPlan(subset1, Double.MAX_VALUE, Integer.MAX_VALUE, null);
        			}
        		}
        	}
        }
        Set<LogicalJoinNode> allNodes = new HashSet<LogicalJoinNode>(joins);
        Vector<LogicalJoinNode> result = pc.getOrder(allNodes);
        if (explain){
